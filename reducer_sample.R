...
while( TRUE ){
	...
	## break this apart into the key and value that were assigned in the Map task
	tuple <- unlist( strsplit( currentLine , "\t" ) )
	currentKey <- tuple[1]
	currentValue <- tuple[2]

	if( ( currentKey != lastKey ) ){
		## a little extra logic here, since the first time through, this conditional will trip
